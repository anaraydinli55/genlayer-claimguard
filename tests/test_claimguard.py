"""
ClaimGuard Contract Tests — unittest (No pip install needed!)
...
# Create fake genlayer package hierarchy
...
fake_web.render = MagicMock(
    return_value="<html><body>Test content found here</body></html>"
)
fake_web.get = MagicMock(
    return_value="Test content found here"
)
...
# Backward compatibility for any older test references
sys.modules["genlayer.std"] = ModuleType("genlayer.std")
sys.modules["genlayer.std._wasi"] = fake_gl


# Preserve the real GenLayer modules so this mock does not leak into other pytest modules.
...
# Remove only the fake modules created by this test.
# Keep any real GenLayer SDK modules loaded by pytest.
...
class TestClaimGuard(unittest.TestCase):
    ...
    def setUp(self):
        ...
        fake_gl.message.sender_address = "0x1234567890abcdef"
        fake_nondet.exec_prompt = MagicMock(return_value='{"verdict":"VERIFIED","confidence":0.85,"reasoning":"ok","evidence_summary":"ok"}')

    def test_init(self):
        c = ClaimGuard()
        c.init()
        self.assertEqual(c.owner, "0x1234567890abcdef")
        self.assertTrue(c.resolvers["0x1234567890abcdef"])
    ...
    def test_create_claim(self):
        ...
        self.assertEqual(c.claims[1].status, "pending")
        self.assertEqual(c.claims[1].category, "fact_check")
    ...
    def test_resolve_claim_verified(self):
        ...
        self.assertEqual(c.claims[cid].status, "verified")
        self.assertEqual(c.claims[cid].votes_for, 1)
        self.assertIsNotNone(c.claims[cid].resolution)

    def test_resolve_claim_rejected(self):
        c = ClaimGuard()
        c.init()
        fake_nondet.exec_prompt = MagicMock(return_value='{"verdict":"REJECTED","confidence":0.95,"reasoning":"No match","evidence_summary":"Different"}')
        cid = c.createClaim("https://example.com", "Wrong", "Verify", "fact_check")
        status = c.resolveClaim(cid)
        self.assertEqual(status, "rejected")
        self.assertEqual(c.claims[cid].votes_against, 1)

    def test_resolve_claim_inconclusive_low_confidence(self):
        c = ClaimGuard()
        c.init()
        fake_nondet.exec_prompt = MagicMock(return_value='{"verdict":"VERIFIED","confidence":0.5,"reasoning":"Maybe","evidence_summary":"Unclear"}')
    ...
    def test_appeal_claim(self):
        ...
        self.assertEqual(c.claims[cid].status, "pending")
        self.assertEqual(c.claims[cid].appeal_count, 1)
    ...
    def test_resolver_management(self):
        ...
        self.assertTrue(c.resolvers["0xnewresolver"])
        c.removeResolver("0xnewresolver")
        self.assertFalse(c.resolvers["0xnewresolver"])
    ...
    def test_get_claim(self):
        ...
        self.assertEqual(claim["url"], "https://example.com")
